...
//!
//! The file is `~/.agentguard/guardrails.yaml`,
//! `<project>/.agentguard/guardrails.yaml`, or `$AGENTGUARD_GUARDRAILS`.
...
pub(crate) const REDACTION: &str = "[AgentGuard guardrail: removed]";
...
impl Guardrails {
    ...
    /// One human-readable line per rule, for `agentguard guardrails list`.
    ...
    /// A commented starter file — `agentguard guardrails example`.
...
pub fn default_paths(cwd: &Path) -> Vec<std::path::PathBuf> {
    let mut out = Vec::new();
    if let Ok(p) = std::env::var("AGENTGUARD_GUARDRAILS") {
        out.push(std::path::PathBuf::from(p));
    }
    out.push(cwd.join(".agentguard").join("guardrails.yaml"));
    if let Some(h) = dirs::home_dir() {
        out.push(h.join(".agentguard").join("guardrails.yaml"));
    }
    out
}
