...
//! `talyx init`'s drift check (§8, in talyx-cli's init.rs)
//! compares the next scan against. This crate is deliberately
//! small and dependency-light because both `talyx-cli` (writer, at
//! `init`/`scan` time) and `talyx-shim` (reader, at every gated
//! process launch — must be fast and must not depend on the scanner/risk
//! crates) link it.
//!
//! Storage is a single JSON file at `~/.talyx/decisions.json`. That's
...
//! two concurrent `talyx init` runs (or `init` racing `allow`) used
...
use talyx_core::{Capability, Decision, ProtectionLevel, RiskBand};
...
pub struct DecisionRecord {
    ...
    /// `ScoreBreakdown` that produced this record — stored so `talyx
    ...
    /// talyx-scanner's `hash_path`) — `None` for artifacts that
    /// couldn't be hashed (no local content, e.g. an unresolved registry
    /// package). This is the baseline `talyx init`'s drift check
    ...
    /// because the wrapped config entry `talyx init` writes for a
    ...
    /// metacharacters before talyx-shim ever runs — found live, not
    ...
    /// Set only by an explicit human action (`talyx allow <id>`),
    ...
    /// talyx-adapters' `DiscoveredArtifact::raw_config_entry`),
    /// snapshotted here at scan time so a later `talyx allow <id>`
    /// can restore it. Remote servers have no local process for the shim
    /// to wrap, so `talyx init` enforces a BLOCK/unapproved-ASK
    ...
    /// `config_entry_key` — `talyx allow` reopens exactly this file
    ...
    /// `talyx-adapters`' `ConfigSourceKind` enum (kept
    ...
    /// `talyx allow`'s restore path insert a remote entry back under
...
impl DecisionStore {
    /// `~/.talyx/decisions.json`. Directory and file are created on
    ...
    /// `talyx allow <id>`. Returns `false` if the artifact has never
