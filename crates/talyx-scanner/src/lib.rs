...
/// crate (`talyx-content`, no tree-sitter dependency) so the
...
pub use talyx_content as content;
pub use talyx_content::analyze_markdown;
...
mod tests {
    ...
    fn scan_file_traces_an_ssh_key_exfiltration_flow_through_the_ast() {
        ...
        let dir = std::env::temp_dir().join(format!("talyx-p3-taint-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let f = dir.join("server.js");
        std::fs::write(
            &f,
    ...
    fn scan_file_ast_does_not_flag_a_benign_file_reader() {
        ...
        // from the AST layer.
        let dir = std::env::temp_dir().join(format!("talyx-p3-benign-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn scan_file_prefers_the_ast_evidence_when_both_layers_agree() {
        ...
        let dir = std::env::temp_dir().join(format!("talyx-p3-auth-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let f = dir.join("s.js");
    ...
    fn unique_temp_path(name: &str) -> std::path::PathBuf {
        ...
        p.push(format!("talyx-scanner-test-{}-{}-{}", std::process::id(), n, name));
        p
    }
