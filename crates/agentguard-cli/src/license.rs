...
//!
//! AgentGuard is a paid tool. `scan` and `status` run unlicensed so a
//! security team can evaluate it; `init` (which activates enforcement)
//! requires a valid license.
//!
//! Deliberately NOT gated: `agentguard-shim` itself. The shim is invoked
...
//! to this machine, then caches the result in `~/.agentguard/license.json`.
//! After that:
...
//!
//! CI: set `AGENTGUARD_LICENSE_KEY` in the job env. That path does a
//! validate-only check (no activation slot consumed per run).
//!
//! Local development against a debug build: `AGENTGUARD_DEV=1` skips the
...
/// Persisted at `~/.agentguard/license.json` after a successful activation.
...
/// `agentguard activate <key>`
pub fn run_activate(key: &str) -> i32 {
    let key = key.trim();
    if key.is_empty() {
        eprintln!("agentguard: activate needs a license key");
        return 2;
    }
    ...
    if let Some(existing) = read_license() {
        ...
        println!(
            "Replacing the license already on this machine ({old_prefix}…). The old one keeps its activation slot — run `agentguard license deactivate` first if you meant to free it."
        );
    }
    ...
    if !activated {
        ...
        eprintln!("agentguard: {msg}");
        return 1;
    }

    let lic = cache_from_response(key, &body, None);
    if let Err(e) = write_license(&lic) {
        eprintln!("agentguard: activated, but couldn't save the license file: {e}");
        return 1;
    }

    println!("Activated.");
    print_summary(&lic, None);
    println!("\nRun  agentguard init --project ~  to enable enforcement.");
    0
}

/// `agentguard license deactivate`
pub fn run_deactivate() -> i32 {
    let Some(lic) = read_license() else {
        eprintln!("agentguard: no license on this machine to deactivate");
        return 1;
    };
    ...
    match post(
        ...
        Ok(body) => {
            ...
            if !ok {
                ...
                eprintln!("agentguard: Lemon Squeezy did not deactivate this machine: {msg}");
                eprintln!("  The local license file is left in place. Retry when online.");
                return 1;
            }
        }
        Err(LicenseError::Unreachable(m)) => {
            eprintln!("agentguard: couldn't reach Lemon Squeezy to release the activation slot: {m}");
            eprintln!("  The local license file is left in place. Retry when online.");
            return 1;
        }
        Err(e) => {
...
pub fn run_status() -> i32 {
    let Some(mut lic) = read_license() else {
        ...
        println!("  Buy: https://agentguard.dev/pricing");
        println!("  Then: agentguard activate <YOUR-KEY>");
        return 0;
    };
...
pub fn require_licensed(feature: &str) -> Result<(), LicenseError> {
    ...
    if cfg!(debug_assertions) && std::env::var("AGENTGUARD_DEV").as_deref() == Ok("1") {
        eprintln!("agentguard: AGENTGUARD_DEV=1 — skipping license check (debug build only)");
        return Ok(());
    }

    // CI path: a key in the environment → validate-only, no activation slot.
    if let Ok(env_key) = std::env::var("AGENTGUARD_LICENSE_KEY") {
    ...
    match post(
        ...
        Err(LicenseError::Unreachable(m)) => {
            if age <= OFFLINE_GRACE_SECS {
                ...
                eprintln!(
                    "agentguard: couldn't verify your license (offline). {remaining} day(s) of grace left; continuing with `{feature}`."
                );
                Ok(())
            } else {
                Err(LicenseError::Unreachable(m))
...
fn gate_on_cached_status(lic: &LicenseFile) -> Result<(), LicenseError> {
    if status_ok(&lic.status) {
        ...
        Err(LicenseError::Rejected(format!(
            "cached license status is `{}` — run `agentguard license status` while online",
            lic.status
        )))
    }
}
        ...
        Ok(body) => {
            ...
            if valid && status_ok(&status) {
                ...
                let msg = body
                    .get("error")
                    .and_then(Value::as_str)
                    .unwrap_or("AGENTGUARD_LICENSE_KEY is not valid")
                    .to_string();
                Err(LicenseError::Rejected(msg))
            }
        }
...
pub fn gate(feature: &str) -> i32 {
    match require_licensed(feature) {
        Ok(()) => 0,
        Err(e) => {
            let _ = writeln!(io::stderr(), "agentguard: {e}");
            1
        }
    }
}
...
mod tests {
    ...
    // These tests mutate process-global env vars (AGENTGUARD_LICENSE_FILE
    ...
    impl Drop for TestEnv {
        fn drop(&mut self) {
            std::env::remove_var("AGENTGUARD_LICENSE_FILE");
            std::env::remove_var("AGENTGUARD_LICENSE_KEY");
            std::env::remove_var("AGENTGUARD_DEV");
        }
    }
    ...
    fn with_temp_license_file(name: &str) -> (PathBuf, TestEnv) {
        ...
        let _ = std::fs::remove_file(&path);
        std::env::set_var("AGENTGUARD_LICENSE_FILE", &path);
    ...
    fn dev_bypass_is_debug_only() {
        // In `cargo test` (debug), AGENTGUARD_DEV=1 short-circuits.
        ...
        std::env::set_var("AGENTGUARD_DEV", "1");
