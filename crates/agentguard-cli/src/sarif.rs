//! SARIF 2.1.0 output for `agentguard scan --format sarif`.
//!
...
//! emitting it lets AgentGuard gate a pull request the same way a linter
//! or SAST tool does, without any AgentGuard-hosted service. The document
...
use agentguard_core::{ArtifactKind, Decision};
use serde_json::{json, Value};
use std::path::Path;

const SCHEMA: &str = "https://json.schemastore.org/sarif-2.1.0.json";
// Placeholder, consistent with the install scripts' `your-org/agentguard`
// until a real repository exists.
const INFO_URI: &str = "https://github.com/your-org/agentguard";
...
fn rule_for(kind: ArtifactKind) -> Value {
    let (name, short, full) = match kind {
        ArtifactKind::McpServer => (
            ...
            "AgentGuard statically analyzed this MCP server's launch command / package / remote endpoint and its declared and inferred capabilities scored above the active protection level's threshold. Review the message for the specific evidence (secret access, outbound network, process execution, reputation).",
        ),
        ArtifactKind::Skill => (
            "RiskySkill",
            "A skill was scored above the allowed risk threshold",
            "AgentGuard analyzed this skill's bundled code and its SKILL.md / instruction text (prompt-injection phrasing, hidden Unicode, encoded payloads, exfiltration directives). A skill's markdown is injected directly into the agent's context, so instruction-manipulation content in it is treated as high severity.",
        ),
        ArtifactKind::Hook => (
            "RiskyHook",
            "An agent hook's command was scored above the allowed risk threshold",
            "AgentGuard analyzed this hook's shell command. Hooks execute automatically on agent events, so a command touching secret material or making outbound network calls is high severity.",
        ),
        ArtifactKind::AgentConfig => (
            "RiskyAgentInstructionFile",
            "An agent-instruction file contains prompt-injection or hidden content",
            "AgentGuard content-scanned this agent-instruction file (.cursorrules / GEMINI.md / AGENTS.md / ...). Its text is fed straight into the agent's context; instruction-override phrasing, text hidden from a human reviewer, or an exfiltration directive here is a supply-chain risk (e.g. a poisoned file committed to a shared repository).",
        ),
        _ => (
            "RiskyArtifact",
            "An agent artifact was scored above the allowed risk threshold",
            "AgentGuard statically analyzed this artifact and it scored above the active protection level's threshold. See the message for the specific evidence.",
        ),
    };
    json!({
        "id": format!("agentguard/{}", kind_slug(kind)),
        "name": name,
...
fn result_for(s: &ScannedArtifact, project_root: &Path) -> Option<Value> {
    ...
    let mut result = json!({
        "ruleId": format!("agentguard/{}", kind_slug(s.artifact.kind)),
        "level": level,
        "message": { "text": message },
        "locations": locations,
        "partialFingerprints": { "agentguardArtifactId/v1": s.artifact.id },
        "properties": {
...
mod tests {
    ...
    fn produces_valid_top_level_shape() {
        ...
        let run = &log["runs"][0];
        assert_eq!(run["tool"]["driver"]["name"], "AgentGuard");
        assert_eq!(run["results"].as_array().unwrap().len(), 1);
        let r = &run["results"][0];
        assert_eq!(r["level"], "error");
        assert_eq!(r["ruleId"], "agentguard/skill");
        assert_eq!(
        ...
        assert_eq!(run["tool"]["driver"]["rules"][0]["id"], "agentguard/skill");
    }
