//! agentguard-advisories — the known-bad feed (BUILD_PLAN.md §7, the
//! "known-bad" half).
//!
...
//! `~/.agentguard/advisories.json` (see [`load`]).
//!
...
impl Advisories {
    ...
    /// Load `~/.agentguard/advisories.json` if present and valid, else the
    /// embedded feed. Used by `agentguard scan`/`init`.
    pub fn load(path: Option<&Path>) -> Advisories {
        ...
        match serde_json::from_str::<Feed>(&text) {
            Ok(feed) if feed.version == 1 => Advisories {
                list: feed.advisories,
                source: "~/.agentguard/advisories.json",
            },
            _ => Advisories {
                list: EMBEDDED_ADVISORIES.list.clone(),
