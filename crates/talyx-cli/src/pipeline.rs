...
use talyx_adapters::{all_adapters, ConfigSource, DiscoveredArtifact, LaunchCommand};
use talyx_core::{
...
/// Location of an optional refreshed advisory feed. `TALYX_ADVISORIES`
/// overrides, else `~/.talyx/advisories.json`. `None` (no home dir) or
/// a missing/invalid file falls back to the feed embedded in the binary —
/// see `talyx_advisories::Advisories::load`.
pub(crate) fn advisories_file() -> Option<PathBuf> {
    std::env::var_os("TALYX_ADVISORIES")
        .map(PathBuf::from)
        .or_else(|| dirs::home_dir().map(|h| h.join(".talyx").join("advisories.json")))
}
...
pub struct ScannedArtifact {
    ...
    /// (currently: MCP servers) — see talyx-adapters' doc comment on
    /// `DiscoveredArtifact`.
    pub launch: Option<LaunchCommand>,
    pub config_source: Option<ConfigSource>,
    /// The original config entry for a remote MCP server (see
    /// talyx-adapters' `DiscoveredArtifact::raw_config_entry` doc
    /// comment) — carried through so `init.rs` can snapshot it into the
    /// decision store for `talyx allow` to restore from later.
...
pub fn collect(
    ...
    let mut out = Vec::new();
    let advisories =
        talyx_advisories::Advisories::load(advisories_file().as_deref());
    ...
    for adapter in all_adapters() {
        ...
        for discovered in adapter.discover(project_root) {
            ...
            if let Some(root) = &scan_root {
                if root.is_dir() {
                    let result = talyx_scanner::scan_dir(root);
                    artifact.capabilities.extend(result.findings);
                    let pkg_json = root.join("package.json");
                    if pkg_json.exists() {
                        artifact
                            .capabilities
                            .extend(talyx_scanner::scan_package_json(&pkg_json));
                    }
                } else if root.is_file() {
                    if let Ok(findings) = talyx_scanner::scan_file(root) {
...
/// `talyx_scanner::shadowing`. Adds a `ToolShadowing` capability
...
fn apply_tool_shadowing(
    ...
    let findings = {
        let refs: Vec<talyx_scanner::shadowing::ServerRef> = mcp_indices
            .iter()
            .map(|&i| talyx_scanner::shadowing::ServerRef {
                name: &out[i].artifact.name,
                source: &out[i].artifact.source,
                trusted: out[i].breakdown.reputation_discount > 0,
            })
            .collect();
        talyx_scanner::shadowing::detect_shadowing(&refs)
    };
...
fn maybe_fetch_registry_package(
    ...
    let cache_dir = talyx_registry::resolve_cache_dir();
    match talyx_registry::fetch_and_extract(source, &cache_dir) {
        Ok(fetched) => {
