//! `talyx init` — BUILD_PLAN.md §5a's config-rewrite enforcement,
//! plus `talyx allow`/`why` for the manual-approval flow that goes
//! with it. This is the only code in the CLI that writes anything: the
//! decision cache (`~/.talyx/decisions.json` by default) and, for MCP
...
use talyx_adapters::ConfigSourceKind;
use talyx_core::{ArtifactKind, ArtifactSource, Capability, Decision, ProtectionLevel, RiskBand};
use talyx_risk::RiskEngine;
use talyx_store::{DecisionRecord, DecisionStore};
use std::collections::{BTreeMap, BTreeSet};
use std::io;
use std::path::{Path, PathBuf};

/// `--store` flag > `$TALYX_STORE` > `~/.talyx/decisions.json`.
...
/// for `talyx allow`'s restore, which may run against a config the
...
/// Finds `talyx-shim(.exe)` next to the running `talyx` binary.
...
fn locate_shim() -> Option<PathBuf> {
    ...
    let dir = exe.parent()?;
    let candidate = dir.join(format!("talyx-shim{}", std::env::consts::EXE_SUFFIX));
...
///
/// `shim_path` is `None` when `talyx-shim` couldn't be located next
...
/// `talyx init --live`, ADR 0001) tells the shim to run the real
...
/// user's file are not preserved (the `.talyx-backup` written before
...
/// `talyx allow` can restore it via `restore_remote_entry_value`.
...
/// time (`record_for`'s `remote_entry_snapshot`), so `talyx allow`
...
/// comment). Parses leniently (talyx_adapters::codex's
...
/// `talyx-shim` and `mcp_config.rs`'s `unwrap_shim_invocation` expect,
...
fn walk_and_rewrite_hook_commands(
    ...
    match value {
        serde_json::Value::Object(map) => {
            ...
            // instead of "command" — see talyx_adapters::
            ...
            for field in talyx_adapters::HOOK_COMMAND_FIELDS {
...
pub fn run_init(
    ...
    for s in &scanned {
        ...
        if let Err(e) = upsert_preserving_approval(&store, record) {
            eprintln!(
                "talyx: failed to write decision cache for '{}': {e}",
                sanitize_for_display(&s.artifact.name)
            );
        }
    }
    ...
    for (config_path, artifacts) in &by_config {
        match rewrite_config(config_path, artifacts, shim_path.as_deref(), live, &store) {
            ...
            Err(e) => {
                eprintln!("talyx: failed to rewrite {}: {e}", config_path.display());
            }
        }
    }
    ...
    if live {
        println!(
            "  --live: MCP servers run through the stdio proxy — the session's JSON-RPC traffic is inspected (ADR 0001). TALYX_NO_PROXY=1 disables it per-launch."
        );
    }
...
pub fn run_allow(artifact_id: &str, store_override: Option<PathBuf>) {
    ...
    match store.approve(artifact_id) {
        ...
        Ok(false) => {
            eprintln!(
                "talyx: '{artifact_id}' has never been scanned. Run `talyx scan` or `talyx init` first."
            );
            std::process::exit(1);
        }
        Err(e) => {
            eprintln!("talyx: failed to update the decision cache: {e}");
            std::process::exit(1);
        }
    }
}

/// After approving a remote MCP server, put its entry back if `talyx
...
fn restore_remote_entry_if_needed(store: &DecisionStore, artifact_id: &str) {
    ...
    match restore_remote_entry(
        ...
        Err(e) => eprintln!(
            "talyx: approved, but failed to restore the config entry at {}: {e}",
            config_path.display()
        ),
    }
}
...
fn restore_quarantined_skill_if_needed(store: &DecisionStore, artifact_id: &str) {
    ...
    if original.exists() {
        eprintln!(
            "talyx: approved, but {} already exists -- not overwriting it. The quarantined copy is still at {}.",
            original.display(),
            current.display()
        );
        return;
...
mod tests {
    use super::*;
    use talyx_adapters::{ConfigSource, LaunchCommand};
    use talyx_core::{Artifact, ArtifactKind, ArtifactSource, PublisherIdentity, ScoreBreakdown};
    ...
    fn rewrite_config_json_leaves_a_manually_approved_ask_entry_in_place() {
        ...
        // Simulate a prior `talyx allow` -- the raw engine decision
    ...
    fn quarantine_skills_moves_a_blocked_skill_and_leaves_an_allowed_one() {
        ...
        assert_eq!(
            ...
            Some(dir.join(".claude").join(".talyx-quarantine").join("evil-skill"))
        );
