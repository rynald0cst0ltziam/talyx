...
//! the common `Artifact` model from talyx-core. Adapters only discover;
//! they never decide or enforce (that's talyx-risk and the future
...
/// specifically so talyx-cli's `init.rs` rewrite path can iterate the
...
pub struct DiscoveredArtifact {
    ...
    /// what `talyx init` needs to rewrite a config entry to route
    ...
    /// config entirely when blocked, and this snapshot is what `talyx
...
/// The config shapes `talyx init` knows how to rewrite. Deliberately
/// an enum, not a trait/callback, so the rewrite logic stays centralized
/// and auditable in one place (talyx-cli) rather than scattered across
...
pub enum ConfigSourceKind {
    ...
    /// talyx-cli's init.rs walks the tree in that identical order to
    ...
    /// returns `None` for this variant, so `talyx init` does not
