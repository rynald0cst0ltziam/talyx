...
fn parse_openhands_config(path: &Path, base_dir: &Path) -> Vec<DiscoveredArtifact> {
    ...
    for key in ["sse_servers", "shttp_servers"] {
        ...
        // Transport prefix on the synthetic entry_key so `talyx init`
        // (removal) and `talyx allow` (restore) know which `[mcp]`
...
"#,
        ...
        assert!(secure
            ...
            .capabilities
            .iter()
            .any(|c| matches!(c.capability, talyx_core::Capability::ApiKeys)));
