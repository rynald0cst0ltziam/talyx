...
//! server (`npx <pkg>`, `uvx <pkg>`, etc — `talyx_core::ArtifactSource
//! ::Registry`) so `talyx-scanner` can run the same static analysis
...
//! from what's already on disk). `talyx scan`/`init` only do this
...
//! `talyx-cli`'s `pipeline.rs` for where that flag is threaded
//! through.
//!
...
//! `~/.talyx/registry-cache/<registry>/<name>/<version>/content/`,
...
pub enum RegistryError {
    ...
    /// implemented (the only two `talyx-adapters`' `classify_command`
...
pub struct FetchedPackage {
    ...
    /// straight to `talyx_scanner::scan_dir` the same way any other
    ...
    /// runs on `talyx_scanner::hash_path(extracted_dir)`, identical
