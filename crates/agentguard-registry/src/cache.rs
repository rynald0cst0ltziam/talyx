...
/// `$AGENTGUARD_REGISTRY_CACHE` env var if set, else
/// `~/.agentguard/registry-cache` — same resolution shape as
/// `agentguard-store`'s `DecisionStore::resolve` (env var override, then a
...
pub fn resolve_cache_dir() -> PathBuf {
    if let Ok(path) = std::env::var("AGENTGUARD_REGISTRY_CACHE") {
        return PathBuf::from(path);
    }
    dirs::home_dir()
        .map(|h| h.join(".agentguard").join("registry-cache"))
        .unwrap_or_else(|| PathBuf::from(".agentguard-registry-cache"))
}
...
fn complete_marker(package_dir: &Path) -> PathBuf {
    package_dir.join(".agentguard-fetch-complete")
}
